...
public class ValCodeController {
    ...
    @RequestMapping(value = "/verifyCodeServlet", method = {RequestMethod.POST, RequestMethod.GET})
    ...
    public void valcode2(HttpServletRequest req, HttpServletResponse resp) {
        initxuan();
        // 定义图像buffer
        BufferedImage buffImg = new BufferedImage(width, height,
                BufferedImage.TYPE_INT_RGB);
        ...
        // 将四位数字的验证码保存到Session中。
        HttpSession session = req.getSession();
        session.removeAttribute("validateCode2");
        session.setAttribute("validateCode2", randomCode.toString());
        //允许跨域访问
        resp.setHeader("Access-Control-Allow-Origin", "*");
        // 禁止图像缓存。
        resp.setHeader("Pragma", "no-cache");
